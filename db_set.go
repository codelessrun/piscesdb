...
func (db *PiscesDB) SAdd(key []byte, members ...[]byte) (res int, err error) {
...
func (db *PiscesDB) SPop(key []byte, count int) (values [][]byte, err error) {
...
func (db *PiscesDB) SIsMember(key, member []byte) bool {
...
func (db *PiscesDB) SRandMember(key []byte, count int) [][]byte {
	db.setIndex.mu.RLock()
	defer db.setIndex.mu.RUnlock()
...
func (db *PiscesDB) SRem(key []byte, members ...[]byte) (res int, err error) {
	if err = db.checkKeyValue(key, members...); err != nil {
		return
	}
...
func (db *PiscesDB) SMove(src, dst, member []byte) error {
	db.setIndex.mu.Lock()
	defer db.setIndex.mu.Unlock()
...
// SCard 返回集合中的元素个数
func (db *PiscesDB) SCard(key []byte) int {
	if err := db.checkKeyValue(key, nil); err != nil {
		return 0
	}
...
// SMembers 返回集合中的所有元素
func (db *PiscesDB) SMembers(key []byte) (val [][]byte) {
	if err := db.checkKeyValue(key, nil); err != nil {
		return
	}
...
func (db *PiscesDB) SUnion(keys ...[]byte) (val [][]byte) {
...
// SDiff 返回给定集合数据的差集
func (db *PiscesDB) SDiff(keys ...[]byte) (val [][]byte) {
	if keys == nil || len(keys) == 0 {
		return
	}
