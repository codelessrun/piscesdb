...
func (db *PiscesDB) LPush(key []byte, values ...[]byte) (res int, err error) {
...
func (db *PiscesDB) RPush(key []byte, values ...[]byte) (res int, err error) {
	if err = db.checkKeyValue(key, values...); err != nil {
		return
	}
...
// LPop 取出列表头部的元素
func (db *PiscesDB) LPop(key []byte) ([]byte, error) {
...
// RPop 取出列表尾部的元素
func (db *PiscesDB) RPop(key []byte) ([]byte, error) {
...
func (db *PiscesDB) LIndex(key []byte, idx int) []byte {
...
//返回成功删除的元素个数
func (db *PiscesDB) LRem(key, value []byte, count int) (int, error) {
...
func (db *PiscesDB) LInsert(key string, option list.InsertOption, pivot, val []byte) (count int, err error) {
...
//bool返回值表示操作是否成功
func (db *PiscesDB) LSet(key []byte, idx int, val []byte) (bool, error) {
...
func (db *PiscesDB) LTrim(key []byte, start, end int) error {
...
func (db *PiscesDB) LRange(key []byte, start, end int) ([][]byte, error) {
	db.listIndex.mu.RLock()
	defer db.listIndex.mu.RUnlock()
...
func (db *PiscesDB) LLen(key []byte) int {
